...
- **`mvn` is not recognized** — Maven isn't installed or isn't on your PATH. Install Apache Maven
  and restart Command Prompt.
- **`BUILD FAILURE` with a red compiler error** — read the first error line; it usually names the
  exact file and line number. Copy that error to ask for help; don't change dependency versions.
- **Dependency download errors** — you need an internet connection the first time you build, so
  Maven can pull Camel/ActiveMQ/Jackson from Maven Central.
- **Program seems to hang** — Task 3 has an intentional ~3–4 second pause (waiting for the
  aggregation timeout); Task 5 has short pauses between test bookings. This is expected, not a
  freeze.
- **Port or "address already in use" errors** — close any other running copy of the program first;
  each task starts its own embedded broker.

## 13. Notes / Design Choices Made Where the PDF Was Not Fully Explicit

- Test booking data (passenger names, fares, dates) was invented for realism since the PDF didn't
  specify exact values beyond the booking IDs and destinations — only the IDs/destinations/field
  names that the PDF explicitly required were treated as fixed requirements.
- Task 3's parts are sent as small JSON fragments per part type (`BOOKING`, `PAYMENT`, `TRIP`)
  over the `booking.parts` queue, correlated by a `bookingId` header, matching the PDF's
  description of "Booking System (BOOKING part), Payment System (PAYMENT part), Trip Assignment
  (TRIP part)".
- Task 5 uses a separate request queue (`marsline.booking.requests.task5`) instead of reusing
  Task 1's queue name, so each task stays fully independent and runnable on its own.
